...
class Trainer:
    ...
    def inference(self, dataloader, additional_outputs=None,
                  return_patient_ids=False, criteria  = 2) -> Dict[str, float]:
        """Model inference.
        ...
        for data in tqdm(dataloader, desc="Evaluation"):
            ...
            with torch.no_grad():
                ...
                n_visit = output["num_visits"].cpu().numpy()
                loss_all.append(loss.item())
                y_true_all.append(y_true)
                y_prob_all.append(y_prob)
                num_visit_all.append(n_visit)
                if additional_outputs is not None:
        ...
        n_visit_all = np.concatenate(num_visit_all, axis=0)
        #print(len(n_visit_all), np.sum(n_visit_all < 3))
        outputs = [y_true_all, y_prob_all, loss_mean, y_true_all[n_visit_all < 3], y_prob_all[n_visit_all < 3]]
        if additional_outputs is not None:
    ...
    def evaluate(self, dataloader) -> Dict[str, float]:
        ...
        if self.model.mode is not None:
            y_true_all, y_prob_all, loss_mean, y_true_all_cold, y_prob_all_cold = self.inference(dataloader)
            mode = self.model.mode
            metrics_fn = get_metrics_fn(mode)
            scores = metrics_fn(y_true_all, y_prob_all, metrics=self.metrics)
            print('scores_normal', metrics_fn(y_true_all, y_prob_all, metrics=self.metrics))
            print('scores_cold', metrics_fn(y_true_all_cold, y_prob_all_cold, metrics=self.metrics))
